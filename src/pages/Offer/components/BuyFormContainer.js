...
function BuyFormContainer() {
  ...
  // const [customerIp, setCustomerIp] = useState("123.123.123.123");
  // const [merchantPosId, setMerchantPosId] = useState("459912");
  // const [txnid, setTxnid] = useState("12345")
  ...
  const [secretKey, setSecretKey] = useState(
    "7f60f24a1573881fe8ea9e19698265d4"
  );
  const [hash, setHash] = useState("");

  useEffect(() => {
    const formDataToHash = `${formData.customerIp}
      ${formData.txnid}
      ${formData.totalAmount}
      ${formData.currencyCode}
      ${formData.notifyUrl}
      ${formData.continueUrl}
      ${formData.firstName}
      ${formData.LastName}
      ${formData.email}
      ${formData.products[0].name} 
      ${formData.products[0].unitPrice} 
      ${formData.products[0].quantity} 
              ${secretKey}`;
    const hash = CryptoJS.SHA256(formDataToHash).toString();
    console.log(hash);
    setHash(
      `sender=${formData.merchantPosId};algorith=SHA-256;signature=${hash}`
    );
  }, [formData, secretKey]);
  const handleSubmit = () => {
    fetch("https://secure.snd.payu.com/api/v2_1/orders", {
      ...
      .then((response) => {
        if (response.ok) {
          console.log("Dane zostały wysłane do PayU");
      ...
      .then((data) => {
        console.log(data);
        // tutaj mozesz juz wykorzystać odpowiedz z serwera i wyswietlic np. komunikat sukcesu
      })
      .catch((error) => {
        console.error(error);
      });
  ...
  return (
    ...
    <select id="country" name="country" class="form-control" required>
      ...
      <form
        method="POST"
        action="https://secure.snd.payu.com/api/v2_1/orders"
        onSubmit={handleSubmit}
      >
        <input type="hidden" name="customerUp" value={formData.customerIp} />
        <input
          type="hidden"
          name="merchantPosId"
          value={formData.merchantPosId}
        />
        ...
        <input
          ...
          value={formData.products[0].unitPrice}
        />
        <input
          type="hidden"
        ...
        <input type="hidden" name="continueUrl" value={formData.continueUrl} />
        <input type="hidden" name="txnid" value={formData.txnid} />
        <input type="hidden" name="OpenPayu-Signature" value={hash} />
