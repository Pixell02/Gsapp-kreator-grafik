...
const PlayerNameProperties = ({ coords, canvasRef, setCoords, handleInputChange, handleSelectChange }) => {
      ...
      <div className="d-flex w-100 flex-row mt-2" style={{ fontSize: "10px" }}>
        ...
        <div className="d-flex flex-column w-100">
          ...
          <select
            ...
            onChange={(e) => handleSelectChange(e)}
          >
            {fonts && fonts.map((team) => <option value={team.value} key={team.value}>{team.label}</option>)}
          </select>
        </div>
      </div>
      <div className='w-100'>
