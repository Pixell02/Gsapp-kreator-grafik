    ...
    <div className='modal-container'>
      <div className='modal-window h-25 p-3'>
        ...
        <div className='d-flex flex-column'>
          ...
          <input type='text' placeholder={selectedTheme.theme} value={themeName} onChange={(e) => setThemeName(e.target.value)} />
        </div>
        <div className='btn-container w-100 d-flex align-items-end'>
          <div className='w-100 d-flex justify-content-end'>
          <button className='btn' onClick={setIsOpen}>Anuluj</button>
          <button className='btn' onClick={() => handleDeleteTheme()} disabled={themeName !== selectedTheme.theme}>Usuń</button>
          </div>
        </div>
      </div>
    </div>
